...
cb_load_conf(const char * fname, const int check_nodef, const int prefix_dir)
{
	...
	if(prefix_dir) {
		snprintf(buff, (size_t)COB_SMALL_MAX, "%s/%s", cob_config_dir, fname);
		name = buff;
	} else {
		name = fname;
	}
	...
	if(fp == NULL) {
		fflush(stderr);
		fprintf(stderr, "%s: %s", name, _("No such file or directory"));
		fflush(stderr);
		return -1;
	}
	...
	while(fgets(buff, COB_SMALL_BUFF, fp)) {
		...
		*s = 0;
		...
		void * var = config_table[i].var;
		const char * val = config_table[i].val;
		...
		case CB_STRING:
			...
			if(strcmp(name, "include") == 0) {
				...
				noreserve * noresptr = (noreserve *) cobc_main_malloc(sizeof(noreserve) + size + 1U);
				noresptr->noresword = (char *)noresptr + sizeof(noreserve);
	...
	if(check_nodef) {
		for(size_t i = 2U; i < CB_CONFIG_SIZE; i++) {
