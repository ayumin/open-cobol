...
013800 77  a                     pic s9(5) comp  value zero.
013800 77  a1                    pic s9(5) comp  value zero.
013900 77  b                     pic s9(5) comp  value zero.
014000 77  c                     pic s9(5) comp  value zero.
014100 77  d                     pic s9(5) comp  value zero.
014200 77  e                     pic s9(5) comp  value zero.
014300 77  s                     pic s9(5) comp  value zero.
014400 77  t                     pic s9(5) comp  value zero.
014500 77  y                     pic s9(5) comp  value zero.
014600 77  y2                    pic s9(5) comp  value zero.
014700 77  z                     pic s9(5) comp  value zero.
014800 77  q                     pic s9(5) comp  value zero.
014800 77  q2                    pic s9(5) comp  value zero.
014900*
...
020100*
020200* Note that SourceInWS has extra 7 chars for free format source.
020300*   and no I don't remember why
020400*
020500 01  SourceInWS.
020600     03  cobolseqno        pic x(6).
...
023700     03  H1-dd             pic xx.
023800     03  filler            pic x     value "/".
023900     03  H1-MM             pic xx.
024000     03  filler            pic x     value "/".
024100     03  H1-YY             pic x(4).
024200     03  filler            pic x     value "@".
024300     03  H1-HH             pic xx.
024400     03  filler            pic x     value ":".
024500     03  H1-Min            pic xx.
...
032400     03  WS-WC-YY          pic x(4).
032500     03  WS-WC-MM          pic xx.
032600     03  WS-WC-DD          pic xx.
032700     03  WS-WC-HH          pic xx.
032800     03  WS-WC-Min         pic xx.
...
034600     03 Msg16     pic x(66)  value "Error: Eof on source possible 
034700-              "logic error at aa047 ASSUMING again".
034800*
034900 01  SectTable.
...
038000*   but xref treats all as being reserved except 'E' (see below)
038100*     as I don't know how to handle that one
038200*
...
101900 01  Resvd-Table-Size        pic s999   comp    value 526.
102000*
102100 01  Condition-Table                           value high-values.
102200     03  Con-Tab-Blocks occurs 250 to 5001
...
102700 01  Con-Tab-Size          pic s9(5) comp value 250.
102800 01  Con-Tab-Count         pic s9(5) comp value zero.
102900*
103000 01  Global-Item-Table                         value high-values.
103100     03  Git-Elements  occurs 250 to 10001
...
104000 01  Git-Table-Size        pic s9(5) comp value 250.
104100 01  Git-Table-Count       pic s9(5) comp value zero.
104200/
...
122600*
122700*` no need to test table size, there can't be 250+ let alone 5000
122800*
...
138800              call "CBL_DELETE_FILE" using "Part2.tmp"
138900              call "CBL_DELETE_FILE" using "Part1.tmp".
139000*
139100     if       End-Prog
...
139900     goback.
140000*> one of these is right for standalone and being called by cobc (C code).
140100     exit     program.
140200     stop     run.
140300/
...
142100              move zero to Global-Current-Level  *> Global only for these
142200     else
142300      if      wsFoundWord2 (1:3) = "CD " or = "SD "   *> and not for these
...
143000                                or = "RD " or = "CD "
143100              perform zz110-Get-A-Word thru zz110-Exit   *> get filename
...
144500          close Source-Listing SourceInput Supplemental-Part1-Out
144600          stop run. *> if here, its broke
144800*
...
146700      display "Source = " SourceInWS (1:60).
146800       close Source-Listing SourceInput Supplemental-Part1-Out
146900       stop run. *> if here, its broke
151100*
...
155100              add 250 to    Con-Tab-Size.
...
159100        stop run. *> if here, its broke & I've cocked it again
159400*
...
184200     if       wsFoundNewWord2 (1:s) not numeric
...
237000     move     spaces to cobolseqno.
237100     move     function upper-case (SourceRecIn)
237200                   to SourceIn8-160.
237300* change tabs to spaces prior to printing
237400*  & remove OC comment lines eg '#'
237500*
...
256800*     Now that code is all free taken from fn.i this lot shouldnt be needed
256900*   nope need to start at cc1 and fn.i can start cc1 and cc2.
257000*
...
257500     end-perform
257600*     if       d > zero    *> d currently pointing to first trailing space
257700*              subtract 1 from d.
257800     if       d < 1
...
282400*
282500* setup printer filename and source rebuild filename
282600*
...
283500              String-Pointer < 7
283600              display Prog-Name
283700              display "Copyright (C) 1967-2009 Vincent Coen"
283800              display " "
...
294500              add 250 to Git-Table-Size.
