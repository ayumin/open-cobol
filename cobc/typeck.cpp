				  ...
				  const int numindex)
{
	...
	if(!cb_relaxed_syntax_check || current_program->decimal_point != ',') {
		return;
	}
	if(numsubs > numindex) {
		return;
...
cb_list_system(void)
{
	...
	for(const system_table * psyst = system_tab; psyst->syst_name; psyst++) {
		...
		size_t n = strlen(psyst->syst_name);
		const char * s;
		switch(n / 8) {
		case 0:
			s = "\t\t\t\t";
...
cb_build_program_id(cb_tree name, cb_tree alt_name, const cob_u32_t is_func)
{
	...
	if(alt_name) {
		current_program->orig_program_id =
			(char *) cobc_check_string((char *)CB_LITERAL(alt_name)->data);
		s = cb_encode_program_id((char *)CB_LITERAL(alt_name)->data);
	} else if(CB_LITERAL_P(name)) {
		current_program->orig_program_id =
			(char *) cobc_check_string((char *)CB_LITERAL(name)->data);
		s = cb_encode_program_id((char *)CB_LITERAL(name)->data);
	} else {
		current_program->orig_program_id =
			(char *) cobc_check_string(CB_NAME(name));
...
cb_build_address(cb_tree x)
{
	...
	int refsubs = cb_list_length(r->subs);
	int numsubs;
	if(CB_FIELD_P(v)) {
	...
	if(!r->flag_all && r->subs) {
		...
		for(cb_field * p = f; p; p = p->parent) {
			...
			if(CB_EXCEPTION_ENABLE(COB_EC_BOUND_SUBSCRIPT)) {
				if(p->depending) {
					cb_tree e1 = CB_BUILD_FUNCALL_4("cob_check_odo",
													cb_build_cast_int(p->depending),
													cb_int(p->occurs_min),
													cb_int(p->occurs_max),
													CB_BUILD_STRING0((CB_FIELD_PTR(p->depending)->name)));
					cb_tree e2 = CB_BUILD_FUNCALL_4("cob_check_subscript",
					...
					if(!CB_LITERAL_P(sub)) {
						cb_tree e1 = CB_BUILD_FUNCALL_4("cob_check_subscript",
														cb_build_cast_int(sub),
														cb_int1,
														cb_int(p->occurs_max),
														CB_BUILD_STRING0(name));
...
cb_validate_program_data(cb_program * prog)
{
	...
	for(cb_tree l = cb_depend_check; l; l = CB_CHAIN(l)) {
		...
		for(cb_field * p = q; p->parent; p = p->parent) {
			 ...
			 cb_tree convention)
{
	...
	for(cb_tree l = Using; l; l = CB_CHAIN(l), numargs++) {
		cb_tree x = CB_VALUE(l);
		if(x == cb_error_node) {
			error_ind = 1;
			continue;
	...
	if(CB_LITERAL_P(prog)) {
		...
		const char * p = (const char *)CB_LITERAL(prog)->data;
		for(; *p; ++p) {
			if(*p == '/' || *p == '\\') {
				entry = p + 1;
			}
				...
				cb_attr_struct * attr_ptr)
{
	...
	for(cb_tree l = values; l; l = CB_CHAIN(l)) {
		cb_tree x = CB_VALUE(l);
		if(x == cb_error_node) {
			return;
		}
	...
	if((CB_REF_OR_FIELD_P(x)) &&
		...
		cb_tree p = CB_BUILD_FUNCALL_3("cob_display", upon, no_adv, values);
...
cb_build_display_name(cb_tree x)
{
	...
	/* Allow direct reference to a device name */
	cb_tree sys = lookup_system_name(name);
	if(sys) {
		switch(CB_SYSTEM_NAME(sys)->token) {
		case CB_DEVICE_CONSOLE:
		case CB_DEVICE_SYSOUT:
...
warning_destination(cb_tree x)
{
	...
	if(!strcmp(f->name, "RETURN-CODE") ||
		...
		cb_warning_x(f, _("'%s' defined here as USAGE %s"),
					 f->name, f->pic->orig);
	} else if(f->usage == CB_USAGE_FLOAT) {
		cb_warning_x(f, _("'%s' defined here as USAGE FLOAT"),
					 f->name);
	} else if(f->usage == CB_USAGE_DOUBLE) {
		cb_warning_x(f, _("'%s' defined here as USAGE DOUBLE"),
					 f->name);
	} else if(f->usage == CB_USAGE_LONG_DOUBLE) {
		cb_warning_x(f, _("'%s' defined here as USAGE FLOAT EXTENDED"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_BIN32) {
		cb_warning_x(f, _("'%s' defined here as USAGE FLOAT-BINARY-7"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_BIN64) {
		cb_warning_x(f, _("'%s' defined here as USAGE FLOAT-BINARY-16"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_BIN128) {
		cb_warning_x(f, _("'%s' defined here as USAGE FLOAT-BINARY-34"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_DEC64) {
		cb_warning_x(f, _("'%s' defined here as USAGE FLOAT-DECIMAL-16"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_DEC128) {
		cb_warning_x(f, _("'%s' defined here as USAGE FLOAT-DECIMAL-34"),
					 f->name);
	} else if(f->pic) {
		cb_warning_x(f, _("'%s' defined here as PIC %s"),
					 cb_name(f), f->pic->orig);
	} else {
		cb_warning_x(f, _("'%s' defined here as a group of length %d"),
					 cb_name(f), f->size);
	}
}
...
count_pic_alphanumeric_edited(cb_field * field)
{
	...
	int count = 0;
	for(unsigned char * p = (unsigned char *)(field->pic->str); *p; p += 5) {
		if(*p == '9' || *p == 'A' || *p == 'X') {
			int repeat;
			memcpy(&repeat, p + 1, sizeof(int));
			count += repeat;
		}
	}
	return count;
...
cb_emit_rewrite(cb_tree record, cb_tree from, cb_tree lockopt)
{
	...
	cb_tree file = CB_FIELD(cb_ref(record))->file;
...
cb_emit_write(cb_tree record, cb_tree from, cb_tree opt, cb_tree lockopt)
{
	...
	cb_tree check_eop;
	if(current_statement->handler_id == COB_EC_I_O_EOP && current_statement->handler1) {
		check_eop = cb_int1;
	} else {
		check_eop = cb_int0;
	}
